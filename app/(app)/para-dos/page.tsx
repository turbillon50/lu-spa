...
export default function ParaDosPage() {
  return (
    <div style={{ background: 'var(--ivory)' }}>
      ...
      {/* Hero full */}
      <div style={{ position: 'relative', height: '70vw', maxHeight: 520, background: '#EDE6D9', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: 28, left: 24, right: 24 }}>
          <p style={{ fontFamily: 'var(--font-pinyon)', fontSize: 30, color: 'rgba(232,213,168,0.9)', marginBottom: 8, letterSpacing: '0.01em' }}>Compartir es el lujo más grande.</p>
          ...
          <p style={{ fontFamily: 'var(--font-montserrat)', fontSize: 13, color: 'rgba(254,252,248,0.75)', lineHeight: 1.65, maxWidth: 280 }}>
      ...
      <section style={{ padding: '0 24px 40px' }}>
        <div style={{ background: 'rgba(237,230,217,0.5)', border: '1px solid rgba(201,169,107,0.15)', borderRadius: 16, padding: '22px 20px' }}>
      ...
      <section style={{ padding: '0 24px 40px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {['Aniversarios', 'Cumpleaños', 'San Valentín', 'Amigos', 'Mama e hija', 'Solo porque sí'].map((tag) => (
            <span key={tag} style={{ fontFamily: 'var(--font-montserrat)', fontSize: 12, color: 'var(--espresso)', background: 'rgba(237,230,217,0.6)', border: '1px solid rgba(201,169,107,0.18)', borderRadius: 24, padding: '6px 16px' }}>{tag}</span>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '0 24px 60px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <Link href="/reservar?t=para-dos&name=Experiencia%20para%20Dos&price=5800&duration=120" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--espresso)', color: '#FEFCF8', padding: '16px', borderRadius: 24, textDecoration: 'none', fontFamily: 'var(--font-montserrat)', fontSize: 13, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600 }}>
