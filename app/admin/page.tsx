      ...
      <div className="admin-dash-grid">
        ...
        <div>
          ...
          <section style={{ padding: '0 22px 24px' }}>
            <p style={{ fontFamily: 'var(--font-montserrat)', fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(201,169,107,0.55)', fontWeight: 500, marginBottom: 14 }}>Ocupación semanal (%)</p>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 80 }}>
              {days.map((day, i) => (
                <div key={day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: '100%', background: 'rgba(201,169,107,0.1)', borderRadius: 4, overflow: 'hidden', height: 60 }}>
                    <div style={{ width: '100%', background: occupancy[i] > 85 ? '#C9A96B' : 'rgba(201,169,107,0.45)', borderRadius: 4, height: `${occupancy[i]}%`, marginTop: 'auto', transition: 'height 0.6s cubic-bezier(.22,1,.36,1)' }} />
                  </div>
                  <span style={{ fontFamily: 'var(--font-montserrat)', fontSize: 9, color: 'rgba(254,252,248,0.35)', letterSpacing: '0.04em' }}>{day}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section style={{ padding: '24px 22px 30px' }}>
            ...
            {clientasRecientes.map((c) => (
              <div key={c.nombre} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(201,169,107,0.06)', padding: '10px 0' }}>
                ...
                <p style={{ fontFamily: 'var(--font-montserrat)', fontSize: 11, color: 'rgba(254,252,248,0.3)', fontVariantNumeric: 'tabular-nums' }}>{c.ultimaVisita}</p>
              </div>
            ))}
          </section>
        </div>
